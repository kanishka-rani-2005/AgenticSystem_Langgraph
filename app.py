...
from rag import add_document_to_rag, thread_has_documents
...
async def upload_document(
    ...
    try:
        allowed_extensions = [".pdf", ".docx", ".txt", ".md", ".py", ".csv" ,".json"]
...
async def chat_stream(request: Request):
    ...
    selected_model = data.get("model") or "gemini-3.5-flash-lite"
    ...
    try:
        if thread_has_documents(thread_id):
            document_instruction = (
                "The user has uploaded documents in this conversation. For every question "
                "that could be answered from those documents, you MUST call "
                "search_uploaded_documents first and then answer with its results."
            )
    except Exception:
        # A document lookup failure must not block a normal chat response.
        pass
    ...
    def event_generator():
        ...
        try:
            ...
            if document_instruction:
                messages.append(SystemMessage(content=document_instruction))
